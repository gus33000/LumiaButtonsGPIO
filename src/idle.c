...
--*/
{
    ...
    *Pending = FALSE;
...

    {
        ...
        WDF_WORKITEM_CONFIG_INIT(&workitemConfig, BtnIdleIrpWorkitem);
...
Routine Description:
    ...
    This is a workitem routine that BtnProcessIdleRequest queues when 
    handling the HIDClass's idle notification IRP, so the idle callback can be made in
    a different thread context, instead of the Idle Irp's dispaBtn call.
...
{ 
    ...
    {
        //
        // IdleQueue is a manual-dispaBtn, non-power-managed queue. This should
        // *never* fail.
        //
